...
export function filterData(data: NowPlayingMessage): PlaybackData | null {
  ...
  const playbackData: NonNullable<PlaybackData> = {
    isPlaying,
    volume,
  ...
  playbackData.supportedActions.push('seek')
...
class NativeHandler extends BasePlaybackHandler {
  ...
  async seek(positionMs: number): Promise<void> {
    ...
    // The native addon reports progress/duration in 100ns ticks (see
    // filterData dividing by 10000), so convert milliseconds back to ticks.
    await this.instance.seekTo(Math.max(0, Math.round(positionMs)) * 10000)
  }
